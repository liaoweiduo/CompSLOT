...
def split_images_labels(imgs):
    ...
    for item in imgs:
        ...
        if len(item) == 3: 
            others.append(item[2])  # e.g., image_id in COCO dataset
    
    # If images is a list of lists/tuples and the first inner element is not numeric or string
    if len(images) > 0 and isinstance(images[0], (list, tuple)):
        first_inner = images[0][0] if len(images[0]) > 0 else None
        if not isinstance(first_inner, (int, float, np.integer, np.floating, str, bytes)):
            images = np.array(images, dtype=object)
    else: 
        images = np.array(images)
    
    if len(others) > 0:
        return images, np.array(labels), np.array(others)
