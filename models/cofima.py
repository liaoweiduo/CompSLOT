...
class Learner(BaseLearner):
    ...
    def incremental_train(self, data_manager):
        ...
        logging.debug(f'self._class_means_slca: {self._class_means_slca.shape} {self._class_means_slca[0, :10]}')
        logging.debug(f'self._class_covs_slca: {self._class_covs_slca.shape} {self._class_covs_slca[0, :10, :10]}')
    ...
    def getFisherDiagonal(self, train_loader, optimizer):
        ...
        for i, (_, inputs, targets) in enumerate(train_loader):
            ...
            loss, res = self._get_loss(inputs, targets, stage=2)
            logits = res['logits']
    ...
    def _get_loss(self, inputs, targets, stage=1, crct_num=None, model=None): 
        if stage == 1:
        ...
        elif stage == 2:  # for fisher
            logits = self._network(inputs, bcb_no_grad=self.fix_bcb)['logits']
            loss = torch.nn.functional.cross_entropy(logits, targets)

        else:
            raise Exception("_get_loss ERROR")
    ...
    def _run(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for epoch in range(1, run_epochs+1):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                loss, res = self._get_loss(inputs, targets, stage=1)
                logits = res['logits']
        ...
        for epoch in range(run_epochs):
            ...
            for _iter in range(crct_num):
                ...
                if self.logit_norm is not None:
                    ...
                    logits = decoupled_logits

                else:
                    loss = F.cross_entropy(logits[:, :crct_num], tgt)
                    logits = logits[:, :crct_num]

                # loss, res = self._get_loss(inp, tgt, stage=2, crct_num=crct_num)
