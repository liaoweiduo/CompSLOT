...
class Learner(BaseLearner):
    ...
    def replace_fc(self, trainloader, model, args):       
        ...
        with torch.no_grad():
            for i, batch in enumerate(trainloader):
                ...
                _, res = self._get_loss(data, label, model=model)
                embedding = res['features_backbone']        # [bs, 768 or 128]
    ...
    def incremental_train(self, data_manager):
        ...
        train_dataset = data_manager.get_dataset(np.arange(self._known_classes, self._total_classes),source="train", mode="train", m_rate=m_rate)
    ...
    def _train(self, train_loader, test_loader, train_loader_for_protonet):
        ...
        logging.debug(f"fc in_features: {self._network.fc.in_features}")

    def _get_loss(self, inputs, targets, wslots=None, model=None): 
        if model is None:
            model = self._network   
            
        if wslots is not None:
            # use wslots as the feature to do classification
            res = model(inputs, features=wslots)
        else:
            res = model(inputs)
